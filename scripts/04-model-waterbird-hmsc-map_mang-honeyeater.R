...
# make predictions of spp. prob of occurrence where CWL? is at max value, given known environmental gradient
predY2 <- predict(m, XData=XDataNew2, studyDesign=m$studyDesign,
                  ranLevels=m$ranLevels, expected=TRUE)

# make predictions of spp. prob of occurrence where N? is at max value, given known environmental gradient
predY3 <- predict(m, XData=XDataNewN, studyDesign=m$studyDesign,
                  ranLevels=m$ranLevels, expected=TRUE)
...
# except for nitrogen we expect a decrease in probability of occurrence, so subtract from baseline instead for nitrogen - so we get decreased prob. of detection from baseline
EpredY$Probability <- rescale(EpredY2$Mangrove.Honeyeater - EpredY$Mangrove.Honeyeater, to = c(0,1)) # rescale from 0-1 for easier visualisation
EpredY$Probability2 <- rescale(EpredY$Mangrove.Honeyeater - EpredY3$Mangrove.Honeyeater, to = c(0,1))
...
ggplot(plotdf) +
  geom_point(aes(x = Probability, 
                 y = Probability2,
                 colour = group)) +
  xlab('Probability of detection with increased Nitrogen') +
...
ggsave('outputs/mangrove-honeyeater-scenario-correlation.png', width = 5.5, height = 5)
...
tmap_save(mm, 'outputs/map-scenario_mangrove-honeyeater.png', width = 6.666, height = 4)
...
m3
tmap_save(m3, 'outputs/map-scenario_mangrove-honeyeater-categories.png', width = 6, height = 6)
